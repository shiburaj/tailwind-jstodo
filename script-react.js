...
const TodoItem = ({ todo, index, toggleTodo, deleteTodo }) => {
    return (
        <tr key={index} className={'transition duration-300' + (!todo.completed?' odd:bg-orange-100 even:bg-orange-50 ':' bg-green-100 line-through ')}>
            <td className="text-center  px-1 py-2 text-orange-800">{ index +1 }</td>
            <td className=" px-1 py-2 text-orange-800">{ todo.text }</td>
            <td className="text-center  px-1 py-2 text-orange-800 flex gap-3 justify-start">
                {(!todo.completed)?
                <button onClick={(e) => toggleTodo(index, true)}  className="text-orange-600">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                </button>:''}
                {(todo.completed)?
                <button onClick={(e) => toggleTodo(index, false)} className="text-orange-600">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>:''}
                {(todo.completed)?
                <button onClick={(e) => deleteTodo(index)} className="text-orange-600">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
